...
在浏览器里捏、拉、戳软软的海洋伙伴：八爪鱼、墨鱼、鱿鱼、金鱼、鲸鱼、鲨鱼，以及海豹、海龟和螃蟹。
...
- 空格 /「戳一下」：缩腕、轻跳、摊开。
- 所有伙伴默认正面，上方头像统一使用正面图。鱼类以及海豹、海龟和螃蟹的头部随横向拉扯连续转向，拉回起点可回到原朝向；选择伙伴和恢复原状时回到正面。
- 右侧：四种颜色、软胶 / 果冻 / 机械材质、软硬、阻尼；「恢复原状」恢复姿态及默认参数。
- 九种伙伴均可切换材质，保留当前姿态、颜色和饰品；材质与软硬、阻尼独立。八爪鱼的机械模式使用分节外壳和关节，墨鱼和鱿鱼使用分节触腕与头部护甲，金鱼、鲸鱼和鲨鱼使用分块外壳和分节鳍片；深色连接层保留弹性，外壳跟随捏拉和转向。机械八爪鱼不再是独立角色，旧链接跳转到八爪鱼页面。切换伙伴时保留当前选择的材质，「恢复原状」回到软胶材质。
- 支持触屏指针输入；滑块可用方向键调整。空格在滑块或按钮获得焦点时保留控件本身的键盘行为。

新增伙伴的专属动作：海豹压扁后弹起，短鳍扑腾；海龟缩头后缓缓探出；螃蟹举起双钳。三只均支持独立拉动附肢、三种材质和现有饰品。
...
- `src/characters/coastalmochi.ts`：海豹、海龟和螃蟹的独立造型、专属动作及共用弹簧形变。
...
海豹、海龟和螃蟹的图鉴封面由对应角色网格经 Blender 离线渲染；灯光与网页略有区别。`test:coastal` 覆盖三只的按压、拖拽、释放、附肢拾取、专属动作、材质、饰品及桌面/手机移动边界。
