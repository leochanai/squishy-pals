...
- 空格 /「戳一下」：缩腕、轻跳、摊开。
- 所有伙伴默认正面，上方头像统一使用正面图。鱼类头部随横向拉扯连续转向，拉回起点可回到原朝向；选择伙伴和恢复原状时回到正面。
- 右侧：四种颜色、软胶 / 果冻 / 机械材质、软硬、阻尼；「恢复原状」恢复姿态及默认参数。
- 六种伙伴均可切换材质，保留当前姿态、颜色和饰品；材质与软硬、阻尼独立。八爪鱼的机械模式使用分节外壳和关节，墨鱼和鱿鱼使用分节触腕与头部护甲，金鱼、鲸鱼和鲨鱼使用分块外壳和分节鳍片；深色连接层保留弹性，外壳跟随捏拉和转向。机械八爪鱼不再是独立角色，旧链接跳转到八爪鱼页面。切换伙伴时保留当前选择的材质，「恢复原状」回到软胶材质。
