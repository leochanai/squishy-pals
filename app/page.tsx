...
export default function Home() {
  ...
  useEffect(() => {
    ...
    import('@/src/core/playground').then(async ({ createPlayground }) => {
      const instance = await createPlayground(host.current!, selected, { onStatus: setStatus, onFps: setFps });
  ...
  return (
    <main className="playground-page">
      ...
      <section className="experience" aria-label="软软伙伴互动区">
        ...
        <nav className="companions" aria-label="选择伙伴"><div className="eyebrow">MEET YOUR PALS <span>01</span></div><div>{characters.map(item => <button className="companion selected" key={item.id} aria-pressed={item.id === characterId} onClick={() => setCharacterId(item.id)}><span className="companion-icon" aria-hidden="true">🐙</span><span><strong>{item.englishName}</strong><small>{item.name}</small></span><span className="selected-check"><Check size={12} /></span></button>)}</div></nav>
