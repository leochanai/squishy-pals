...
export default function PlaygroundPage({ characterId }: { characterId: string }) {
  ...
  const palette = basePalette.some(item => item.color === defaultColor) ? basePalette : [{ name: selected.id === 'octomochi' && material === 'mechanical' ? '金属灰' : ({ clownmochi: '小丑鱼橙', goldmochi: '金鱼橙', whalemochi: '深海蓝', sharkmochi: '鲨鱼蓝', crabmochi: '蟹壳红' }[selected.id] ?? '原色'), color: defaultColor }, ...basePalette.slice(1)];
